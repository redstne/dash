...
import {
  ...
  MapPin, Activity, Users, ChevronRight, RefreshCw, Sword, History,
} from "lucide-react";
...
// ── Main page: layout that switches between list and detail via URL ───────────
export function ServerPlayers({ serverId }: { serverId: string }) {
  const id = serverId;
  const navigate = useNavigate();
  const location = useRouterState({ select: (s) => s.location.pathname });
  const isAtList = location === `/servers/${id}/players`;
  if (!isAtList) return <Outlet />;
  return <PlayerListView id={id} onSelect={(name) => void navigate({ to: `/servers/${id}/players/${name}` as "/" })} />;
}
...
function PlayerListView({ id, onSelect }: { id: string; onSelect: (n: string) => void }) {
  ...
  const { data: sessions = [], isLoading: sessionsLoading } = useQuery<PlayerSession[]>({
    queryKey: ["player-sessions", id],
    queryFn: () => fetch(`/api/servers/${id}/players/sessions?limit=100`, { credentials: "include" }).then((r) => r.json()),
    refetchInterval: 30_000,
  });

  function fmtDuration(seconds: number | null): string {
    if (seconds === null) return "Online";
      ...
      <div className="px-6 py-4 border-b border-border flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <TabsList className="h-8">
            <TabsTrigger value="players" className="text-xs h-6 gap-1.5"><Users className="w-3 h-3" />Players</TabsTrigger>
            <TabsTrigger value="history" className="text-xs h-6 gap-1.5"><History className="w-3 h-3" />History</TabsTrigger>
          </TabsList>
                    ...
                    <tr key={s.id} className="border-b border-border/50 last:border-0 hover:bg-muted/20">
                      <td className="px-4 py-2">
                        <div className="flex items-center gap-2">
                          <img src={`https://mc-heads.net/avatar/${s.playerName}/24`} alt={s.playerName}
                            className="w-6 h-6 rounded bg-muted shrink-0"
                            onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                          <span className="font-medium text-xs">{s.playerName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-2">
                        {s.durationSeconds === null ? (
                          <Badge className="bg-green-600/20 text-green-400 border-green-600/30 text-[10px] px-1.5 py-0">Online</Badge>
                        ) : (

...
function storageIcon(type: string) {
  switch (type) {
    ...
    case "gdrive": return <Cloud className="w-4 h-4 text-yellow-400" />;
    default: return <HardDrive className="w-4 h-4 text-green-400" />;
  }
}

const STORAGE_LABELS: Record<string, string> = {
  local: "Local", s3: "S3 / B2 / Wasabi", sftp: "SFTP", rclone: "Rclone Remote", gdrive: "Google Drive",
};
...
interface DestForm {
  name: string;
  storageType: "local" | "s3" | "sftp" | "rclone" | "gdrive";
  schedule: string;
  retentionCount: number;
  // local
  localPath: string;
  ...
  // rclone
  rcloneRemote: string; rclonePath: string; rcloneConfigContent: string;
  // gdrive
  gdriveServiceAccountJson: string; gdriveFolderId: string;
}
...
const EMPTY_FORM: DestForm = {
  ...
  rcloneRemote: "", rclonePath: "minecraft/backups", rcloneConfigContent: "",
  gdriveServiceAccountJson: "", gdriveFolderId: "",
};
...
function formToConfig(f: DestForm) {
  ...
  if (f.storageType === "gdrive") return {
    type: "gdrive" as const,
    serviceAccountJson: f.gdriveServiceAccountJson,
    folderId: f.gdriveFolderId,
  };
  return { type: "rclone" as const, remote: f.rcloneRemote, path: f.rclonePath, configContent: f.rcloneConfigContent || undefined };
}
      ...
      <Dialog open={destOpen} onOpenChange={setDestOpen}>
        <DialogContent className="sm:max-w-[480px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-3 py-2">
            ...
            <div className="grid gap-1.5">
              ...
              <Select value={form.storageType} onValueChange={(v) => setF("storageType", v)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="rclone">🔄 Rclone Remote (Proton Drive, etc.)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            {/* Google Drive */}
            {form.storageType === "gdrive" && (
              <>
                <div className="p-3 rounded-lg bg-yellow-950/30 border border-yellow-600/30 text-xs text-yellow-200 space-y-2">
                  <p className="font-medium text-yellow-300">Google Drive via Service Account</p>
                  <ol className="list-decimal list-inside space-y-1 text-yellow-200/80">
                    ...
                    <li>Create a service account, then generate a JSON key and download it</li>
                    <li>Share your Google Drive backup folder with the service account email (<code className="bg-black/40 px-1 rounded">…@….iam.gserviceaccount.com</code>)</li>
                    <li>Paste the JSON key content below and enter the folder ID from the Drive URL</li>
                  </ol>
                </div>
                <div className="grid gap-1.5">
                  ...
                  <textarea
                    ...
                    value={form.gdriveServiceAccountJson}
                    onChange={(e) => setF("gdriveServiceAccountJson", e.target.value)}
                ...
                <div className="grid gap-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) => setF("gdriveFolderId", e.target.value)}
                    className="bg-black/30 border-red-600/30 font-mono text-xs"
                  />
                  <p className="text-[11px] text-muted-foreground">Found in the Drive folder URL: drive.google.com/drive/folders/<span className="text-yellow-400">FOLDER_ID</span></p>
                </div>
              </>
            )}
              ...
              <>
                <div className="grid gap-1.5">
                  ...
                  <textarea
                    ...
                    value={form.rcloneConfigContent}
                    onChange={(e) => setF("rcloneConfigContent", e.target.value)}
                    className="w-full resize-y rounded-md border border-red-600/30 bg-black/30 px-3 py-2 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-red-600 text-foreground placeholder:text-muted-foreground"
                  />
                  <p className="text-[11px] text-muted-foreground">Paste the contents of your <code className="bg-muted px-1 rounded">rclone.conf</code> here. Leave empty to use the config mounted at <code className="bg-muted px-1 rounded">/root/.config/rclone/rclone.conf</code>.</p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="grid gap-1.5">
                    <Label>Remote Name</Label>
                    <Input placeholder="gdrive" value={form.rcloneRemote} onChange={(e) => setF("rcloneRemote", e.target.value)} className="bg-black/30 border-red-600/30 font-mono text-xs" />
                    <p className="text-[11px] text-muted-foreground">The <code className="bg-muted px-1 rounded">[name]</code> from your config</p>
                  </div>
                  <div className="grid gap-1.5">
                    <Label>Remote Path</Label>
                    <Input placeholder="Backups/minecraft" value={form.rclonePath} onChange={(e) => setF("rclonePath", e.target.value)} className="bg-black/30 border-red-600/30 font-mono text-xs" />
                  </div>
                </div>
              </>
            )}

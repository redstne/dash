...
export function ServerFiles({ serverId }: { serverId: string }) {
  const id = serverId;
  const { path: currentPath, file: openFilePath } = useSearch({ strict: false }) as { path: string; file?: string };
  ...
  const goTo = useCallback((path: string) => {
    void navigate({ search: (prev: Record<string, string | undefined>) => ({ ...prev, path, file: undefined }) });
  }, [navigate]);

  /** Open a file — pushes into history */
  const pushFile = useCallback((filePath: string) => {
    void navigate({ search: (prev: Record<string, string | undefined>) => ({ ...prev, file: filePath }) });
  }, [navigate]);
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: (_, filePath) => {
      ...
      if (openFile?.path === filePath) void navigate({ search: (prev: Record<string, string | undefined>) => ({ ...prev, file: undefined }) });
    },
  });
              ...
              <div className="flex items-center justify-between px-3 py-1.5 border-b border-border bg-card shrink-0 gap-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Button
                    ...
                    onClick={() => { void navigate({ search: (prev: Record<string, string | undefined>) => ({ ...prev, file: undefined }) }); setIsDirty(false); }}
                  >
